...
namespace BillingAPI.Mediatr.Handlers.AccountHandlers
{
    public class LoginHandler : IRequestHandler<LoginCommand, AccountDTO>
    {
        ...
        public async Task<AccountDTO> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            ...
            // check user exists
            Account? user = await _dataContext.Accounts.SingleOrDefaultAsync(a => a.Username.Trim() == request.LoginDTO.Username);
            if (user == null)
